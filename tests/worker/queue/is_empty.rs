...
//!
//! This module verifies the behavior of the is_empty check for determining whether the
//! worker queue contains any jobs. Tests cover empty and non-empty states, interaction
//! with push/pop operations, scheduled jobs, and consistency with the len method.
...
mod is_empty {
    ...
    #[tokio::test]
    async fn returns_true_for_empty_queue() {
    ...
    #[tokio::test]
    async fn returns_false_after_push() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    #[tokio::test]
    async fn returns_true_after_popping_all_jobs() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    #[tokio::test]
    async fn returns_false_after_schedule() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    #[tokio::test]
    async fn is_consistent_with_len_method() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
