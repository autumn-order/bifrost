//! Tests for WorkerQueue::pop method.
//!
//! This module verifies the behavior of the pop operation for retrieving jobs from the
//! worker queue. Tests cover popping from empty queues, retrieving different job types,
//! chronological ordering, and the interaction between scheduled and immediate jobs.
...
mod pop {
    ...
    /// Tests popping from an empty queue returns None.
    ///
    /// Verifies that attempting to pop a job from an empty queue succeeds without
    /// errors and correctly returns None to indicate no jobs are available.
    ///
    /// Expected: Ok(None)
    #[tokio::test]
    async fn returns_none_for_empty_queue() {
    ...
    /// Tests successful retrieval of a single character job.
    ///
    /// Verifies that a character update job can be pushed to the queue and then
    /// successfully popped, returning the correct job with matching data.
    ///
    /// Expected: Ok(Some(UpdateCharacterInfo)) with matching character_id
    #[tokio::test]
    async fn retrieves_single_character_job() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    /// Tests successful retrieval of a single alliance job.
    ///
    /// Verifies that an alliance update job can be pushed to the queue and then
    /// successfully popped, returning the correct job with matching data.
    ///
    /// Expected: Ok(Some(UpdateAllianceInfo)) with matching alliance_id
    #[tokio::test]
    async fn retrieves_single_alliance_job() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    /// Tests successful retrieval of a single corporation job.
    ///
    /// Verifies that a corporation update job can be pushed to the queue and then
    /// successfully popped, returning the correct job with matching data.
    ///
    /// Expected: Ok(Some(UpdateCorporationInfo)) with matching corporation_id
    #[tokio::test]
    async fn retrieves_single_corporation_job() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    /// Tests that popped jobs are removed from the queue.
    ///
    /// Verifies that after successfully popping a job, that job is no longer
    /// present in the queue and subsequent pop attempts return None.
    ///
    /// Expected: First pop returns job, second pop returns None
    #[tokio::test]
    async fn removes_job_from_queue() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
        let job = WorkerJob::UpdateCharacterInfo {
            character_id: 12345,
    ...
    /// Tests that jobs are popped in chronological order (FIFO).
    ///
    /// Verifies that when multiple jobs are scheduled at different times in the past,
    /// they are popped in chronological order (earliest scheduled first), ensuring
    /// proper FIFO queue behavior.
    ///
    /// Expected: Jobs returned in order: 11111, 22222, 33333
    #[tokio::test]
    async fn returns_jobs_in_chronological_order() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
        let now = Utc::now();
    ...
    /// Tests sequential popping of multiple jobs.
    ///
    /// Verifies that multiple jobs can be pushed to the queue and then popped
    /// sequentially, with each pop returning a job until the queue is empty.
    ///
    /// Expected: Each of 5 pops returns a job, then queue is empty
    #[tokio::test]
    async fn handles_multiple_jobs_sequentially() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
    ...
    /// Tests that mixed job types are popped in chronological order.
    ///
    /// Verifies that when different types of jobs (Character, Alliance, Corporation)
    /// are scheduled at different times, they are popped in chronological order
    /// regardless of their type.
    ///
    /// Expected: Jobs returned in chronological order by schedule time
    #[tokio::test]
    async fn returns_mixed_job_types_in_order() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
        let now = Utc::now();
    ...
    /// Tests immediate availability of pushed jobs.
    ///
    /// Verifies that a job pushed with the current timestamp is immediately
    /// available for popping without any delay.
    ///
    /// Expected: Job is available immediately after push
    #[tokio::test]
    async fn pushed_jobs_are_immediately_available() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
        let job = WorkerJob::UpdateCharacterInfo {
            character_id: 99999,
    ...
    /// Tests that future-scheduled jobs are not returned until due.
    ///
    /// Verifies that a job scheduled for future execution is not returned by
    /// pop until its scheduled time has passed, ensuring proper scheduling behavior.
    ///
    /// Expected: Pop returns None for future-scheduled job
    #[tokio::test]
    async fn future_scheduled_jobs_not_returned_until_due() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
        let job = WorkerJob::UpdateCharacterInfo {
            character_id: 55555,
    ...
    /// Tests that past-scheduled jobs are immediately available.
    ///
    /// Verifies that a job scheduled for a time in the past is immediately
    /// available for popping, treating it as already due for execution.
    ///
    /// Expected: Pop returns the past-scheduled job immediately
    #[tokio::test]
    async fn past_scheduled_jobs_are_immediately_available() {
        let redis = RedisTest::new().await.expect("Failed to create Redis test");
        let queue = setup_test_queue(&redis);
        let job = WorkerJob::UpdateCharacterInfo {
            character_id: 77777,
