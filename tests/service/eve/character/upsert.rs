//! Tests for CharacterService::upsert method.
//!
//! This module verifies the character upsert service behavior during ESI data
...
/// Tests upserting a new character without faction.
///
...
#[tokio::test]
async fn upserts_new_character_without_faction() -> Result<(), TestError> {
    ...
    let result = character_service.upsert(character_id).await;
...
/// Tests upserting a new character with faction.
///
...
#[tokio::test]
async fn upserts_new_character_with_faction() -> Result<(), TestError> {
    let character_id = 95_000_001;
    let corporation_id = 98_000_001;
...
/// Tests upserting a character with full hierarchy.
///
...
#[tokio::test]
async fn upserts_character_with_full_hierarchy() -> Result<(), TestError> {
    let character_id = 95_000_001;
    let corporation_id = 98_000_001;
...
/// Tests upserting a character when corporation already exists.
///
...
#[tokio::test]
async fn upserts_character_with_existing_corporation() -> Result<(), TestError> {
    let character_id = 95_000_001;
    let corporation_id = 98_000_001;
...
/// Tests upserting a character when faction already exists.
///
...
#[tokio::test]
async fn upserts_character_with_existing_faction() -> Result<(), TestError> {
    let character_id = 95_000_001;
    let corporation_id = 98_000_001;
    let faction_id = 500_001;
...
/// upserting the character data.
///
/// Expected: Ok with character data upserted after retry
#[tokio::test]
...
///
/// Expected: Ok with character and corporation upserted after retry
#[tokio::test]
...
/// to upsert a character without the required database tables being created.
///
...
///
/// Verifies that the character service returns a database error when attempting
/// to upsert a character with corporation dependency but the corporation table doesn't exist.
///
/// Expected: Err with DbErr
#[tokio::test]
...
///
/// Verifies that the character service returns a database error when attempting
/// to upsert a character with faction dependency but the faction table doesn't exist.
///
/// Expected: Err with DbErr
#[tokio::test]
...
/// Tests upserting multiple characters sequentially.
///
/// Verifies that the character service can successfully upsert multiple different
/// characters in sequence, each with their own data and dependencies.
///
/// Expected: Ok with all characters created
#[tokio::test]
async fn upserts_multiple_characters_sequentially() -> Result<(), TestError> {
    ...
    // Upsert first character
    let result1 = character_service.upsert(char1_id).await;
    assert!(result1.is_ok());

    // Upsert second character (same corporation)
    let result2 = character_service.upsert(char2_id).await;
    assert!(result2.is_ok());

    // Upsert third character (different corporation)
    let result3 = character_service.upsert(char3_id).await;
...
///
/// Verifies that calling upsert multiple times for the same character is idempotent,
...
async fn upsert_is_idempotent() -> Result<(), TestError> {
    ...
    // Upsert same character three times
    let result1 = character_service.upsert(character_id).await;
    assert!(result1.is_ok());

    let result2 = character_service.upsert(character_id).await;
    assert!(result2.is_ok());

    let result3 = character_service.upsert(character_id).await;
    assert!(result3.is_ok());
