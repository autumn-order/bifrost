...
async fn returns_zero_when_no_characters() -> Result<(), TestError> {
    ...
    let result = schedule_character_info_update(&test.state.db, &queue).await;
...
async fn handles_duplicate_scheduling_attempts() -> Result<(), TestError> {
    ...
    // Schedule first time
    let result1 = schedule_character_info_update(&test.state.db, &queue).await;
    ...
    let result2 = schedule_character_info_update(&test.state.db, &queue).await;
