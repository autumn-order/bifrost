...
impl<'a> CharacterRepository<'a> {
    ...
    /// Find character IDs that don't exist in the database
    pub async fn find_missing_ids(&self, character_ids: &[i64]) -> Result<Vec<i64>, DbErr> {
        if character_ids.is_empty() {
            return Ok(Vec::new());
        }

        let existing_characters: Vec<i64> = entity::prelude::EveCharacter::find()
        ...
        let missing_ids: Vec<i64> = character_ids
            .iter()
            .filter(|id| !existing_characters.contains(id))
            .copied()
            .collect();
...
mod tests {
    ...
    mod find_missing_ids {
        ...
        /// Expect empty vector when all characters exist
        #[tokio::test]
        async fn returns_empty_when_all_exist() -> Result<(), TestError> {
            ...
            let char1 = test.eve().insert_mock_character(1, 1, None, None).await?;
            let char2 = test.eve().insert_mock_character(2, 1, None, None).await?;
            let char3 = test.eve().insert_mock_character(3, 1, None, None).await?;

            let character_repo = CharacterRepository::new(&test.state.db);

            let character_ids = vec![char1.character_id, char2.character_id, char3.character_id];
            let result = character_repo.find_missing_ids(&character_ids).await;
        ...
        /// Expect missing IDs when some characters don't exist
        #[tokio::test]
        async fn returns_missing_ids() -> Result<(), TestError> {
            ...
            // Query for characters 1, 2, 3, 4, 5 where only 1 and 3 exist
            let (char2_id, _) = test.eve().with_mock_character(2, 1, None, None);
            let (char4_id, _) = test.eve().with_mock_character(4, 1, None, None);
            let (char5_id, _) = test.eve().with_mock_character(5, 1, None, None);
            ...
            assert!(missing.contains(&char2_id));
            assert!(missing.contains(&char4_id));
            assert!(missing.contains(&char5_id));
            assert!(!missing.contains(&char1.character_id));
            assert!(!missing.contains(&char3.character_id));

            Ok(())
        }

        /// Expect all IDs returned when no characters exist
        #[tokio::test]
        async fn returns_all_when_none_exist() -> Result<(), TestError> {
            ...
            let character_ids = vec![90000001, 90000002, 90000003];
            let result = character_repo.find_missing_ids(&character_ids).await;

            assert!(result.is_ok());
            let missing = result.unwrap();
            assert_eq!(missing.len(), 3);
            assert_eq!(missing, character_ids);
        ...
        async fn fails_when_tables_missing() -> Result<(), TestError> {
            ...
            let character_ids = vec![90000001, 90000002];
            let result = character_repo.find_missing_ids(&character_ids).await;
